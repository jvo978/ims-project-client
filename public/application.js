...
var createItemError = function createItemError() {
...
var getItemsSuccess = function getItemsSuccess(data) {
  var yourItems = data.items.filter(function (item) {
    if (item.owner === store.user._id) {
      return item;
    }
  });
  var showNewItemsHtml = showItemsTemplate({ items: yourItems });
...
var deleteItemError = function deleteItemError() {
  $('#itemTitle').css('color', 'red');
  $('#itemTitle').html('Something went wrong deleting item try again!');
};

var updateItemSuccess = function updateItemSuccess(itemId) {
  $('[data-id="modal' + itemId + '"]').modal('hide');
  $('.modal-backdrop').remove();
  $('.update-form')[0].reset();
};
var updateItemError = function updateItemError() {
...
var store = __webpack_require__(3);
var itemUi = __webpack_require__(4);
...
var apiUrl = void 0;
var apiUrls = {
  production: 'https://ims-project-backend.herokuapp.com',
...
var onCreateItem = function onCreateItem(event) {
  ...
  itemApi.createItem(data).then(itemUi.createItemSuccess).then(onGetItems).catch(itemUi.createItemError);
};
...
var onDeleteItem = function onDeleteItem(event) {
  ...
  itemApi.deleteItem(itemId).then(onGetItems).catch(itemUi.deleteItemError);
};
...
var onUpdateItem = function onUpdateItem(event) {
  ...
  var data = getFormFields(event.target);
  var itemId = $(event.target).attr('data-id');
  itemApi.updateItem(data, itemId).then(function () {
    return itemUi.updateItemSuccess(itemId);
  }).then(onGetItems).catch(itemUi.updateItemError);
};
...
var itemEvents = __webpack_require__(9);
var itemUi = __webpack_require__(4);
...
var onSignIn = function onSignIn(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  authApi.signIn(data).then(authUi.signInSuccess).then(itemEvents.onGetItems).catch(authUi.signInError);
};
...
module.exports = __webpack_require__.p + "e9cb149c03abc5b75f7179ba1687db29.png";
